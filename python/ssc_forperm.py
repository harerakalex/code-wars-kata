...
def ssc_forperm(arr):
    perm =[find_sum(x) for x in set([x for x in list(itertools.permutations(arr, len(arr)))])]
    
    return [{"total perm": len(perm)}, {"total ssc": sum(perm)}, {"max ssc": max(perm)}, {"min ssc": min(perm)}]
